...
// ExactInputParams for SWAP_EXACT_IN (0x07): multi-hop swap
...
// --- Encoding helpers (exported for arb-bot use) ---
...
export function encodeSettleAll(currency: Address, maxAmount: bigint): Hex {
  return encodeAbiParameters(
...
export function encodeTakeAll(currency: Address, minAmount: bigint): Hex {
  return encodeAbiParameters(
    [{ type: "address" as const }, { type: "uint256" as const }],
    [currency, minAmount]
  );
...
export function encodeV4Swap(actions: Hex, params: Hex[]): Hex {
  return encodeAbiParameters(
...
export function encodeUnwrapWeth(recipient: Address, minAmount: bigint): Hex {
  return encodeAbiParameters(
    [{ type: "address" as const }, { type: "uint256" as const }],
    [recipient, minAmount]
  );
...
export function encodePermit2PermitCommand(
